					...
					send: {
						preSend: [
							async function (this: any, requestOptions) {
								...
								// Remove duplicates and add to body
								if (tagsToAdd.length > 0) {
									body.tags = [...new Set(tagsToAdd)];
								}

								const customVarsParam = this.getNodeParameter(
									'customVariables',
									0,
									undefined,
								) as any;
								...
								if (
									trackingParam?.trackingOptions &&
									Array.isArray(trackingParam.trackingOptions)
								) {
					...
					send: {
						preSend: [
							async function (this: any, requestOptions) {
								...
								// Find existing lead
								const existingLead = await this.helpers.httpRequestWithAuthentication!.call(
									this,
									'niApi',
									{
										method: 'GET',
								...
								// If it's an existing lead (PUT)
								if (existingLead?.data?.length > 0) {
									const leadId = existingLead.data[0].id;
									requestOptions.method = 'PUT';
									requestOptions.url = `/leads/${leadId}`;

									// DON'T send phone in PUT (already used to find the lead)
									// Only send the other fields normally
									body.name = name;
									body.email = email;
									body.notes = notes;

									// Process tags separately via specific endpoint
									let tagsToAdd: string[] = [];
									const selectableTags = this.getNodeParameter('selectable_tag', 0, []) as string[];
									if (Array.isArray(selectableTags) && selectableTags.length > 0) {
										tagsToAdd = [...selectableTags];
									...
									// If it's a new lead (POST) - send all fields normally
					...
					send: {
						preSend: [
							async function (this: any, requestOptions) {
								...
								// Remove duplicates and set in body
								if (allTags.length > 0) {
						...
						preSend: [
							async function (this: any, requestOptions) {
								...
								if (allTags.length > 0) {
									...
									throw new Error(
										'At least one tag must be provided (using "Tags" or "Selectable Tags")',
									);
								}
